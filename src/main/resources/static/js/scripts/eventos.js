        ...
        .done((data) => {
            ...
            if (data.ok) {
                $.each(data.body, (index, registro) => {
                    ...
                    t.row.add([botonera, registro.id, registro.nombre, registro.descripcion, registro.fechaInicio.slice(0, 10), registro.fechaFin.slice(0, 10), registro.costo]);
                });
                t.draw(false);
            } else {
    ...
    ajaxRequest("GET", `${url}/${id}`)
        .done((data) => {
            if (data.ok) {
                ...
                $("#fechaFin").val(convertirFecha(data.body.fechaFin));
